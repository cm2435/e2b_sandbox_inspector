"""Sync client for E2B sandbox inspection."""
...
class SandboxInspector:
    """Sync client for inspecting and interacting with E2B sandboxes.

    Usage:
        inspector = SandboxInspector()
        sandboxes = inspector.list_sandboxes()
        result = inspector.exec("sbx_abc", "ls -la")
    """
    ...
    def list_sandboxes(
        ...
        paginator = Sandbox.list(query=query, api_key=self.api_key)
    ...
    def info(self, sandbox_id: str) -> SandboxInfo:
        ...
        """
        sandbox = Sandbox.connect(sandbox_id=sandbox_id, api_key=self.api_key)
        sbx_info = sandbox.get_info()  # type: ignore[call-overload]
    ...
    def metrics(
        ...
        """
        sandbox = Sandbox.connect(sandbox_id=sandbox_id, api_key=self.api_key)
        raw_metrics = sandbox.get_metrics(start=start, end=end)  # type: ignore[call-overload]
    ...
    def python(
        ...
        sandbox = CodeSandbox.connect(sandbox_id=sandbox_id, api_key=self.api_key)
    ...
    def files(self, sandbox_id: str, path: str = "/") -> list[FileInfo]:
    ...
    def download(self, sandbox_id: str, remote_path: str) -> bytes:
    ...
    def upload(self, sandbox_id: str, remote_path: str, content: bytes) -> None:
    ...
    def kill(self, sandbox_id: str) -> bool:
        ...
        """
        try:
            Sandbox.kill(sandbox_id=sandbox_id, api_key=self.api_key)
            return True
    ...
    def kill_all(self, confirm: bool = False) -> int:
